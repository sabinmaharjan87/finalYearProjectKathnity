...
class club extends CI_Controller {
    ...
    function add() {
        ...
        $data['players'] = $this->player_model->getall();
        if ($_POST) {
            $club_image = $this->do_upload();
            $this->form_validation->set_rules('country_name',
                    'country_name',
                    'required');
            $this->form_validation->set_rules('club_name',
                    'club_name',
                    'required');
            $this->form_validation->set_error_delimiters('<font color="#ace123"> <br>',
                    '</font>');
            if ($this->form_validation->run() == FALSE) {
                $this->load->view('club/club_form',
                        $data);
            } else {
                $this->club_model->insert($club_image);
                redirect('club/display');
                ;
            }
        } else {
            $this->load->view('club/club_form',
    ...
    function edit($club_id) {
        ...
        if ($_POST) {
            ...
            if ($newImage != '') {
                ...
                $club = $this->club_model->getsingle($club_id);
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->load->view('club/club_edit_form',
                        $data);
            } else {
                $this->club_model->update($club_id,$club_image);
                redirect('club/display');
            }
        } else {
            $data['club'] = $this->club_model->getsingle($club_id);
    ...
    function display() {
        ...
        $data['countrys'] = $this->country_model->getall();
        $data['players'] = $this->player_model->getall();
        $this->load->view('club/club_table',
                $data);
        $this->load->view('home/includes/footer');
    }
    ...
    function delete($club_id) {
        ...
        if ($_POST) {
            ...
            $this->club_model->delete($club_id);
            redirect('club/display');
        } else {
            $data['club'] = $this->club_model->getsingle($club_id);
            $this->load->view('club/club_delete',
                    $data);
        }
        $this->load->view('home/includes/footer');
    }
